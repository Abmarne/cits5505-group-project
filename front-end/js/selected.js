...
function buildUnitCard(course, altIdx, col, isConflict) {
  ...
  const sessionHTML = sessions
    .map((s) => {
      const typeCls =
        SESSION_TYPE_CLS[s.type.toLowerCase()] ||
  ...
  const tagHTML = [
    ...
    isConflict
      ? `<span class="inline-flex items-center px-[7px] py-[2px] rounded-md text-[10px] font-mono border border-[rgba(247,111,111,.35)] bg-[var(--red-bg)] text-[var(--red)]">Conflict</span>`
      : "",
  ].join("");
