...
/* ── Helpers ───────────────────────────────── */
function stripPassword({ password: _, ...u }) { return u; }
...
function defaultUser(sn) {
  return { timetable: { selected: [], semester: 'S1', name: 'My Timetable', isPublic: false }, friends: [], pending: [], sent: [] };
}
...
/* ── Mock users ────────────────────────────── */
const MOCK_USERS = [
  { id: 1, name: 'Hung Nguyen',  initials: 'HN', email: '21000001@student.uwa.edu.au', studentNumber: '21000001', password: 'demo1234' },
  { id: 2, name: 'Alex Smith',   initials: 'AS', email: '21234567@student.uwa.edu.au', studentNumber: '21234567', password: 'demo1234' },
  { id: 3, name: 'Jordan Lee',   initials: 'JL', email: '21345678@student.uwa.edu.au', studentNumber: '21345678', password: 'demo1234' },
  { id: 4, name: 'Riley Morgan', initials: 'RM', email: '21456789@student.uwa.edu.au', studentNumber: '21456789', password: 'demo1234' },
  { id: 5, name: 'Casey Park',   initials: 'CP', email: '21567890@student.uwa.edu.au', studentNumber: '21567890', password: 'demo1234' },
];
...
const API = {
  ...
  async register({ name, studentNumber, email, password }) {
    if (!email.endsWith('@student.uwa.edu.au')) throw new Error('Must use a UWA student email');
    if (!/^2\d{7}$/.test(studentNumber))        throw new Error('Invalid student number');
    if (password.length < 8)                    throw new Error('Password must be at least 8 characters');
    return { id: Date.now(), name, initials: getInitials(name), email, studentNumber };
  },
  ...
  async changePassword({ currentPassword, newPassword }) {
    if (!currentPassword || newPassword.length < 8) throw new Error('Invalid password');
    return { ok: true };
  },
  ...
  async updateProfile({ name, studentNumber }) {
    const updated = { ...State.getUser(), name, initials: getInitials(name), studentNumber };
  ...
  async autoSchedule({ selected, preferences = {} }) {
    const courses = await this.getCourses();
    const { avoid8am = false, compactDays = false, freeFridays = false } = preferences;
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      for (let alt = 0; alt <= course.alternatives.length; alt++) {
        const test      = result.map((s, j) => j === i ? { ...s, altIdx: alt } : s);
        ...
        const penFri    = (freeFridays && sessions.some(s => s.day  === 4)) ? 10 : 0;
        const penSpread = compactDays ? new Set(sessions.map(s => s.day)).size : 0;
  ...
  async sendFriendRequest(studentNumber) {
    ...
    if (me) {
      ...
      if (!theirData.pending.find(r => r.studentNumber === me.studentNumber)) {
        theirData.pending.push({ ...me, requestedAt: new Date().toISOString() });
      }
    }
    saveDB(db);
    return { ok: true };
  ...
  async cancelFriendRequest(studentNumber) {
    ...
    if (me) {
      const theirData = userDB(db, studentNumber);
      theirData.pending = theirData.pending.filter(r => r.studentNumber !== me.studentNumber);
    }
    saveDB(db);
    return { ok: true };
  },
  ...
  async acceptFriendRequest(studentNumber) {
    ...
    myData.pending = myData.pending.filter(r => r.studentNumber !== studentNumber);
    ...
    if (me) {
      const theirData = userDB(db, studentNumber);
      if (!theirData.friends.find(f => f.studentNumber === me.studentNumber)) {
        theirData.friends.push({ ...me, addedAt: new Date().toISOString() });
      }
      theirData.sent = theirData.sent.filter(r => r.studentNumber !== me.studentNumber);
    }
    saveDB(db);
    return { ok: true };
  },
  ...
  async getFriendTimetable(studentNumber) {
    ...
    const owner = match
      ? stripPassword(match)
      : { name: `Student ${studentNumber}`, initials: getInitials(`Student ${studentNumber}`), studentNumber };
  ...
  async lookupUser(studentNumber) {
    ...
    return { id: Date.now(), name: `Student ${studentNumber}`, initials: getInitials(`Student ${studentNumber}`), email: `${studentNumber}@student.uwa.edu.au`, studentNumber };
  },
  ...
  seedDemoData() {
    ...
    if (!alexData.timetable.isPublic) {
      alexData.timetable = {
        selected: [{ code: 'CITS1001', altIdx: 0 }, { code: 'MATH1001', altIdx: 0 }],
        semester: 'S1',
